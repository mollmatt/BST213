...
The *goal* of this assignment is to compare single vs. muliple predictor models.
...
lbw_quant <- as.data.frame(lbw %>% dplyr::select(quantVars))
...
## Anova method
```{r multiple predictor regression anova method}
...
for(i in 1:length(responseVars)) {
     ...
     print(paste0("Response var: ", responseVars[i]))
     print(paste0("Explanatory var: ", explanVars))

     anovaresult <- aov(as.formula(paste0(responseVars[i],"~",paste0(explanVars,collapse = "+"))), data = lbw)
...
str(summary(anovaresult))

```

## OLS method
```{r multiple predictor regression ols method}


LinMod <- lm(as.formula(paste0(responseVars,"~",paste0(explanVars,collapse = "+"))), data = lbw)
...
SS.residuals <- anova(LinMod)["Residuals", "Sum Sq"]
SS.model <- sum(anova(LinMod)["Sum Sq"]) - anova(LinMod)["Residuals", "Sum Sq"]
SS.total <- sum(anova(LinMod)["Sum Sq"])

## R-squared
summary(LinMod)$r.squared

kable(data.frame(Model_SS = SS.model, Residual_SS = SS.residuals, Total_SS = SS.total))


     ## plot residuals
     print(ggplot(data = lm(as.formula(paste0(responseVars,"~",paste0(explanVars,collapse = "+"))), data = lbw)) + 
             geom_point(aes(x = .fitted, y = .resid)) + 
             labs(title = paste0("Residuals vs fitted"))) # fitted vs residuals
...
LinMod <- lm(as.formula(bwt~lwt+as.factor(ht)+as.factor(race)), data = lbw)
summary(LinMod)

lm.sumSquares(LinMod)

lsmeans(LinMod, pairwise~ht)

lsmeans(LinMod, pairwise~race, adjust = "none") # Note, there is not adjustment for multiple comparisons
...
```{r linear regression coeffienct based method part b}

## First show coefficients of model
summary(LinMod)$coefficients

## Now get partial correlation coefficients
partialR <- pcor(lbw[,c(responseVars,explanVars)], method = "pearson")
partialRtable <- rbind(sapply(partialR$estimate[,responseVars], function(x) x^2),
      partialR$p.value[,responseVars])
rownames(partialRtable) <- c("pcor","p-value")
partialRtable
...
```{r determine effect of each variable part c}

## Determine effect of each variable when added to smoke as model

## Create vector with variables to add in addition to smoke
varToEval <- names(lbw)[!names(lbw) %in% c("id","smoke","bwt","low")]
varToEval

for(j in 1:length(varToEval)) {
  
  LinMod <- lm(as.formula(paste0("bwt~smoke+",paste0(varToEval[j],collapse = "+"))), data = lbw)
  
  ## Evaluate model
  print(summary(LinMod)$coefficients)
  print(" ")
  print(" ")
  
  ## Now get partial correlation coefficients

  partialR <- pcor(lbw[,c(responseVars,"smoke",varToEval[j])], method = "pearson")
  partialRtable <- rbind(sapply(partialR$estimate[,responseVars], function(x) x^2),
      partialR$p.value[,responseVars])
  rownames(partialRtable) <- c("pcor","p-value")
  print(partialRtable)
  ...
  print(" ")
  print(" ")
  print(paste0("R-squared: ",signif(summary(LinMod)$r.squared, 4)))
  print(" ")
  print(" ")
